...
eta = np.linspace(np.finfo(float).eps, 0.7, 50)

seeds = [0j, 2j, 6j, 12j, 20j]
seed = seeds[0:(basis.N[0]+1)]
print(seed)

#eigsys = Eigensystem(ops_list=[H0, Hint], param_list=[alpha, eta], M=50, simult_obs=Spin, simult_seed=seed)
eigsys = Eigensystem(ops_list=[H0, Hint], param_list=[alpha, eta], M=20)
...
dump(savedict,"results/result_simult_Oc_eta_{:d}_{:d}.p".format(basis.m[0], basis.N[0]), compress=3) #scaling