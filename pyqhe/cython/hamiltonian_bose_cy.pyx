...
# ============================================================================
# OPTIMIZED ARRAY-BASED LOOKUP VERSIONS (GIL-FREE)
# ============================================================================

@cython.boundscheck(False)
@cython.wraparound(False)
def linear_fast(data_type_t [:,:] basis, np.uint32_t [:,:] site_indices,
                np.float64_t [:,:] coeff, np.int32_t [:] lut, int max_occ):
    """
    Computes a Hamiltonian of the form
    sum_basis  coeff[j,k] * b^dagger_j  b_k |state>

    OPTIMIZED VERSION: Uses array-based lookup instead of dict (GIL-free).
...
@cython.boundscheck(False)
@cython.wraparound(False)
def quadratic_fast(data_type_t [:,:] basis, np.uint32_t [:,:] site_indices,
...
@cython.boundscheck(False)
@cython.wraparound(False)
def quadratic_delta_fast(data_type_t [:,:] basis, np.float64_t [:,:,:,:] coeff,
