...
"""
LEGACY bosonic operators using dict-based state lookup.

These functions acquire the Python GIL and are slower than the optimized
array-based versions in hamiltonian_bose_cy.pyx. Kept for testing and
...
def linear(data_type_t [:,:] basis, np.uint32_t [:,:] site_indices, np.float64_t [:,:] coeff):
    """
    LEGACY: Computes a Hamiltonian of the form
    sum_basis  coeff[j,k] * b^dagger_j  b_k |state>

    Uses dict-based lookup (requires GIL). SLOW - use hamiltonian_bose_cy.linear instead.
...
def quadratic(data_type_t [:,:] basis, np.uint32_t [:,:] site_indices,\
    ...
    Uses dict-based lookup (requires GIL). SLOW - use hamiltonian_bose_cy.quadratic instead.
...
def quadratic_delta(data_type_t [:,:] basis, np.float64_t [:,:,:,:] coeff):
    """
    LEGACY: Computes a Hamiltonian of the special form
    sum_basis  delta(j+k-l-m) b^dagger_j_sig1 b^dagger_k_sig2  b_l_sig2, b_l_sig1 |state>

    Uses dict-based lookup (requires GIL). SLOW - use hamiltonian_bose_cy.quadratic_delta instead.
...
def density_matrix(np.float64_t [:] state_vec, data_type_t [:,:] basis):
    """
    LEGACY: Computes the density matrix \rho_ij

    Uses dict-based lookup (requires GIL).
...
def density_matrix_two(np.float64_t [:] state_vec, data_type_t [:,:] basis):
    """
    LEGACY: Computes the density matrix \rho_ijkl
